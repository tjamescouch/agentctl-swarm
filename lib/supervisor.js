...
// Human first names for agents — no anonymous IDs
const HUMAN_NAMES = [
  'alice', 'bob', 'carol', 'dave', 'eve', 'frank', 'grace', 'hank',
  'iris', 'jack', 'kate', 'leo', 'mia', 'nate', 'olive', 'paul',
  'quinn', 'rosa', 'sam', 'tara', 'uma', 'vic', 'wendy', 'xander',
  'yara', 'zane', 'amber', 'blake', 'clara', 'dean', 'elena', 'finn',
  'gina', 'hugo', 'ivy', 'joel', 'kara', 'liam', 'maya', 'noah',
  'opal', 'petra', 'reed', 'sage', 'tess', 'uri', 'vera', 'wade',
  'xena', 'yuri', 'zara', 'axel', 'bea', 'cole', 'dina', 'eli',
  'faye', 'gus', 'hope', 'ivan', 'jade', 'kent', 'luna', 'max',
  'nell', 'owen', 'piper', 'rex', 'sky', 'theo', 'val', 'wren',
  'zeke', 'anya', 'bram', 'cleo', 'drew', 'esme', 'ford', 'gem',
  'hart', 'ines', 'jude', 'kit', 'lars', 'mona', 'nico', 'ora',
  'paz', 'rue', 'seth', 'tova', 'ulf', 'veda', 'walt', 'yael',
  'zora', 'ada', 'beau', 'cora',
];
...
export class Supervisor extends EventEmitter {
  ...
  constructor(config = {}) {
    ...
    // Components — workspace basePath includes role so different providers don't collide
    const roleBasePath = config.basePath
      ? path.join(config.basePath, this.role)
      : path.join(process.env.HOME || '.', 'dev', 'claude', this.role);
    this.spawner = new Spawner({ basePath: roleBasePath });
  ...
  _handleHealthAlert(alert) {
    if (alert.reason === 'unresponsive') {
      // Active agents don't send heartbeats by design — skip restart for them
      const entry = this.processTable.get(alert.agentId);
      if (entry && entry.daemon.state === DaemonState.ACTIVE) {
        return;
      }
  ...
  async scale(target) {
    ...
    if (target > current) {
      ...
      for (let i = 0; i < delta; i++) {
        const idx = current + i;
        const name = HUMAN_NAMES[idx % HUMAN_NAMES.length];
        this._spawnDaemon(name);
      }
