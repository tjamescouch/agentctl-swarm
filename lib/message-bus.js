  ...
  async connect() {
    ...
    try {
      if (this.clientModule) {
        ...
        // Try @tjamescouch/agentchat package first
        const attempts = [
          () => import('@tjamescouch/agentchat'),
          // Try global npm prefix (ESM can't use NODE_PATH)
          async () => {
            const { execSync } = await import('child_process');
            const prefix = execSync('npm config get prefix', { encoding: 'utf8' }).trim();
            return import(`${prefix}/lib/node_modules/@tjamescouch/agentchat/dist/lib/client.js`);
          },
          // Fall back to relative path to sibling agentchat repo
          () => import('../../agentchat/dist/lib/client.js'),
        ];
        for (const attempt of attempts) {
          try {
            const mod = await attempt();
            AgentChatClient = mod.AgentChatClient;
            if (AgentChatClient) break;
          } catch { /* try next */ }
        }
        if (!AgentChatClient) throw new Error('All import paths exhausted');
      }
    } catch (err) {
